...
module.exports = async (req, res, next) => {
    ...
    let profileQuery = await pool.query(`SELECT * FROM users WHERE name=$1;`, [userRequestedName]);

    let quizQuery = await pool.query("SELECT * FROM quizgames WHERE id=$1 AND player_id=$2;", [
        userRequestedName,
        currectUserId,
    ]);
    ...
    } else if (quizQuery.rowCount > 0 && quizQuery.rows[0].id === req.body.id) {
        next();
    } else {
